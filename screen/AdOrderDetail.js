...
const AdOrderDetail = ({ navigation, route }) => { ///main
    ...
    useEffect(() => { /// listen order
        ...
        const unsub = onSnapshot(doc(db, "orders", order.id.toString()), (doc) => {
            setShopIdArr([... new Set(doc.data().orderItems.map(item => item.shopId))])
            setOrder(doc.data())
        });
        return () => {
            unsub()
    ...
    function SetOrderStatus(status) { /// shop xác nhận đơn hàng
        try {
            updateDoc(doc(db, 'orders', order.id.toString()), {
                status: status,
                orderItems: order.orderItems.map(item => ({ ...item, orderStatus: status })),
            ...
            if (status == 'cancel') {
                users.filter(u => u.id == order.user.id ||
                    JSON.stringify(order.orderItems).includes(u.phone) || u.id == order.shipper.id)
                    .forEach(u => {
