...
const AdNewFoodOrder = ({ navigation }) => {
    const { orders, currentUser } = useContext(Context)
    const [select, setSelect] = useState('')
    const [searchKey, setSearchKey] = useState('')

    function FilterOrder() {
        let tempOrders = orders.filter(order => order.status.includes(select) && order.type == 'food')
        if (currentUser.role == 'shipper') {
            tempOrders = tempOrders.filter(order => order.shipper.id == currentUser.id || order.status == 'pendding')
        }
        ...
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <FlatList
                ...
                renderItem={({ item }) => <NewOrderFlatlist order={item} onPress={() => {
                    if (currentUser.power.includes('food')|| currentUser.role == 'admin') {
                        navigation.navigate('AdOrderDetail', item)
                    }else{
                        alert('not register food')
                    }
